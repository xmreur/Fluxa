...
export const Inbox = () => {
    ...
    const [projectNames, setProjectNames] = useState({}) // map project_id -> project name
    ...
    useEffect(() => {
        const loadNames = async () => {
            // TEAM NAMES
            const teamIds = invites.filter(i => i.team_id).map(i => i.team_id)
            if (teamIds.length > 0) {
            ...
            // PROJECT NAMES
            const projectIds = invites.filter(i => i.project_id).map(i => i.project_id)
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-3xl text-white font-bold flex items-center gap-3">
                        <lucide.LuInbox className="h-8 w-8 text-blue-500" /> Inbox
                    </h1>
                    <p className="text-gray-400 mt-1">View your invites and notifications</p>
                </div>
                {unreadCount > 0 && (
                    <button
            ...
            <div className="mt-6 min-w-full min-h-fit">
                        ...
                        {invites.length === 0 && (
                            <Card>
                                <CardContent className="flex flex-col items-center justify-center py-12">
                                    ...
                                    <p className="text-gray-400 text-sm">You'll see team and project invites here</p>
                                </CardContent>
                            </Card>
                        )}
                                    ...
                                    <CardContent className="p-4">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <h4 className="font-medium">
                                                        Invitation to join {invite.team_id ? teamNames[invite.team_id] : projectNames[invite.project_id]}
                                                        <span className="ml-2 text-xs text-gray-400">
                                                            ({invite.team_id ? 'Team' : 'Project'})
                                                        </span>
                                                    </h4>
                                                    <div className="flex items-center gap-2 text-sm text-gray-400 mt-1">
                                                        <div className="flex justify-center items-center gap-1">
                                                            Will join as
                                                            <span className="font-bold text-gray-300 ml-1">{capitalize(invite.role)}</span>
                                                        </div>
                                                        <span>•</span>
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                >
                                                    <lucide.LuX className="h-4 w-4" /> Decline
                                                </button>
                                                <button
                                                ...
                                                >
                                                    <lucide.LuCheck className="h-4 w-4" /> Accept
                                                </button>
                                            </div>
                                        </div>
                                    </CardContent>
                ...
                <div className={`space-y-4 ${activeTab === 'notifications' ? 'block' : 'hidden'}`}>
                    <div className="flex items-center justify-between mb-2">
                        <DropdownMenu
                            trigger={
                                <button className="px-4 py-1.5 text-white gap-3 rounded-lg flex justify-center items-center bg-slate-950 border border-slate-700 hover:bg-gray-400/30 cursor-pointer">
                    ...
                    {filteredNotifications.length === 0 ? (
                        <Card>
                            <CardContent className="flex flex-col items-center justify-center py-12">
                                ...
                                <p className="text-gray-400 text-sm">You'll see activity notifications here</p>
                            </CardContent>
                        </Card>
                    ) : (
